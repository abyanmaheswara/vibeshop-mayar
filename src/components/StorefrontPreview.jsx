...
export default function StorefrontPreview({ data, views = 0, isPreviewOnly = false, customSlug = "", onRegenerate }) {
  ...
  return (
    <motion.div id="storefront-preview" ref={storefrontRef} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-5xl mx-auto space-y-12 pb-20 relative p-8 rounded-[3rem] bg-[#080408] text-white">
      <div className="text-center space-y-4">
        <h1
          className="text-5xl md:text-7xl leading-none font-extrabold italic tracking-tighter uppercase"
          style={{
        ...
        {isPreviewOnly && (
          <div className="inline-flex items-center justify-center gap-2 px-4 py-1.5 rounded-full border text-sm font-bold glass border-[#ffffff1a] text-[#ffffff99]">
            <Eye size={16} />
            {views.toLocaleString()} views
          </div>
        )}
        <p className="text-lg max-w-xl mx-auto text-[#ffffff99]">{data.description}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        {data.products.map((product) => (
          <motion.div key={product.id} whileHover={{ y: -10 }} className="rounded-[3rem] overflow-hidden group transition-all duration-500 relative glass border-[#ffffff0d] hover:border-[#00e5ff4d] shadow-2xl">
            <div className="h-96 overflow-hidden relative">
              ...
              <div className="absolute top-6 right-6 px-6 py-2 rounded-full glass border-[#ffffff1a] text-[#00E5FF] neon-glow-cyan italic font-black">Rp {product.price.toLocaleString("id-ID")}</div>
            </div>
            <div className="p-12 space-y-8 relative">
              <div className="space-y-2">
                <h3 className="text-4xl leading-tight transition-colors duration-500 line-clamp-2 font-black italic tracking-tighter uppercase group-hover:text-[#00E5FF]">{product.name}</h3>
                <p className="text-base leading-relaxed max-w-sm line-clamp-2 text-[#ffffff99]">{product.description || "Premium high-quality items designed carefully."}</p>
              </div>
              <NeonButton
                color="cyan"
                className={`w-full !py-5 flex items-center justify-center gap-4 text-xl font-bold tracking-[0.1em] italic uppercase tracking-[0.2em] font-black ${isCheckingOut === product.id ? "opacity-80" : ""}`}
              ...
              >
                ...
                {isCheckingOut === product.id ? "Preparing..." : "Buy Now"}
              </NeonButton>
            </div>
          </motion.div>
        ))}
